...
class PyTealLexerBase(Lexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == PyTealParser.EOF and len(self.indents) != 0:
            # Remove any trailing EOF tokens from our buffer.
            self.tokens = [token for token in self.tokens if token.type != PyTealParser.EOF]

            # First emit an extra line break that serves as the end of the statement.
            self.emitToken(self.commonToken(PyTealParser.NEWLINE, '\n'))
            ...
            self.emitToken(self.commonToken(PyTealParser.EOF, '<EOF>'))

        next_ = super().nextToken()
        return next_ if len(self.tokens) == 0 else self.tokens.pop(0)

    def createDedent(self):
        return self.commonToken(PyTealParser.DEDENT, '')
    ...
    def onNewLine(self):
        ...
        if self.opened > 0 or (next_next != -1 and next_ in (10, 13, 35)):
            ...
            self.emitToken(self.commonToken(PyTealParser.NEWLINE, new_line))
            ...
            if indent == previous:
                ...
                self.emitToken(self.commonToken(PyTealParser.INDENT, spaces))
            else:
